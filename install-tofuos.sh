...
print_success_banner() {
  echo -e "${GREEN}╔═══════════════════════════════════════════════════════════════════════╗${NC}"
  echo -e "${GREEN}║                 TofuOS Installation Successful!                       ║${NC}"
  echo -e "${GREEN}║                                                                       ║${NC}"
...
print_failure_banner() {
  echo -e "${RED}╔═══════════════════════════════════════════════════════════════════════╗${NC}"
  echo -e "${RED}║                 TofuOS Installation Failed!                           ║${NC}"
  echo -e "${RED}║                                                                       ║${NC}"
...
if [ -d "tofuos" ]; then
  ...
  echo -e "${RED}║                                                                       ║${NC}"
  echo -e "${RED}║  An existing TofuOS installation was detected at ~/tofuos             ║${NC}"
  echo -e "${RED}║                                                                       ║${NC}"
  ...
  echo -e "${GREEN}tofuos exists, backing up to .config/tofuos-backups folder.${NC}"
  if [ -d ".config/tofuos-backups" ]; then
    echo -e "${GREEN}Moving current version of TofuOS to backups folder.${NC}"
    mv "$HOME"/tofuos .config/tofuos-backups/"$backupname"
    sleep 1
  else
    echo -e "${GREEN}Creating the backups folder & moving TofuOS to it.${NC}"
    mkdir -p .config/tofuos-backups
    mv "$HOME"/tofuos .config/tofuos-backups/"$backupname"
    sleep 1
  fi
else
  echo -e "${GREEN}Thank you for choosing TofuOS.${NC}"
...
sudo nixos-rebuild boot --flake ~/tofuos/#${profile}
