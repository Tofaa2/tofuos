...
your `tofuos` environment. It provides a comprehensive set of commands to
...
- Edit `~/tofuos/hosts/<HOSTNAME>/variables.nix` and uncomment `mimeDefaultApps`.
...
- Edit `~/tofuos/modules/core/flatpak.nix`
...
- Go to the `~/tofuos/host/HOSTNAME`
...
1. In the file, `~/tofuos/modules/core/system.nix`
...
Monitor settings are in the file: `~/tofuos/hosts/<HOSTNAME>/variables.nix`
...
<details>
<summary>**How do I add applications to TofuOS? **</summary>
...
1. For applications to be included in all defined hosts edit the
   `~/tofuos/modules/core/packages.nix` file.

There is a section that begins with: `environment.systemPackages = with pkgs;`

Followed by a list of packages These are required for TofuOS.
...
`~/tofuos/hosts/<HOSTNAME>/host-packages.nix`
...
<details>
<summary>** I made a change to my TofuOS configuration, how do I activate it? **</summary>

- Use the `zcli` utility. `zcli rebuild`
- The legacy `fr`, Flake Rebuild alias, is depreciated but still available **
  NOTE: If you **created a new file\*\*
- you will need to run a `git add .` command in the `tofuos` folder
...
1. You have to edit the `hardware.nix` file for that host in
   `~/tofuos/hosts/HOSTNAME/hardware.nix` and override the default.
...
- `cp -rpv ~/tofuos/hosts/OLD-HOSTNAME ~/tofuos/hosts/NEW-HOSTNAME`

2. Edit the `~/tofuos/flake.nix` file. Change the line:

- `host = "NEW-HOSTNAME"`

3. In the `~/tofuos` Directory run `git add .` _The rebuild will fail with a
...
1. Edit the `~/tofuos/modules/core/boot.nix` file.
2. Look for:
...
1. Either run the `install-tofuos.sh` script and select `nvidia-laptop`
   template or if configuring manually, set the template in the `flake.nix` to
   `nvidia-prime`

2. In the `~/tofuos/hosts/HYBRID-HOST/variables.nix` file you will need to set
...
4. In the `~/tofuos/modules/home/hyprland/config.nix` file is an ENV
...
1. Edit the `~/tofuos/modules/core/stylix.nix` file.
...
1. Edit the `~/tofuos/hosts/HOSTNAME/varibles.nix`
...
- Wallpapers are stored in the `~/tofuos/wallpapers` directory.
...
1. Edit the `~/tofuos/modules/home/hyprland/config.nix` file.
...
1. Edit the `~/tofuos/modules/home/scripts/wallsetter`
...
```bash
git -C ~/tofuos fetch origin
git -C ~/tofuos show origin/main:upgrade-2.3-to-2.4.sh > ~/upgrade-2.3-to-2.4.sh
...
```bash
curl -fsSL https://gitlab.com/zaney/tofuos/-/raw/main/upgrade-2.3-to-2.4.sh -o ~/upgrade-2.3-to-2.4.sh
chmod +x ~/upgrade-2.3-to-2.4.sh
```
...
1. First backup your existing `tofuos` directory. e.g.
   `cp -r ~/tofuos ~/tofuos-backup`

2. There is no direct update. When you clone the the new config the config files
   and layout have changed.

3. You need to install tofuos like a new install. `./install-tofuos.sh`
...
2. Backup your `tofuos` directory e.g. `cp -r ~/tofuos ~/tofuos-backup`

3. Run the `./install-tofuos.sh` script and follow the new install
   instructions.
...
The Yazi configuration file is located in `~/tofuos/modules/home/yazi.nix`

Yazi is configured like VIM and VIM motions

The keymap is in the `~/tofuos/modules/home/yazi/keymap.toml` file
...
Edit the `/tofuos/modules/home/wezterm.nix` Change `enable = false` to
`enable = true;`\
Save the file and rebuild tofuos with the `fr` command.
...
The kitty bindings are configured in `~/tofuos/modules/home/wezterm.nix`
...
1. Edit the `~/tofuos/modules/home/ghostty.nix` file.
