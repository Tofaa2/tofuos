...
# Define directories
ZANEYOS_DIR="$HOME/tofuos"
TEMP_DIR="/tmp/tofuos-comparison-$$"
TIMESTAMP=$(date +"%Y-%m-%d_%H-%M-%S")
REPORT_FILE="$HOME/tofuos-config-comparison-$TIMESTAMP.txt"

# Default comparison targets
DEFAULT_REPO="https://github.com/Tofaa2/tofuos.git"
DEFAULT_BRANCH="main"
...
show_usage() {
  ...
  echo "                          (default: https://github.com/Tofaa2/tofuos.git)"
  ...
  echo "  $0 -r https://github.com/Tofaa2/tofuos.git -b dev"
...
perform_comparison() {
  ...
  if [ ! -d "$ZANEYOS_DIR" ]; then
    print_error "TofuOS directory not found at $ZANEYOS_DIR"
    exit 1
  fi
